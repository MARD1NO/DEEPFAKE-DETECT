...
"""
CNN+双向LSTM 和 Resnet3D版本的训练主体程序
效果并不是很好
10帧版本，上20帧容易爆显存
"""
...
if __name__ == "__main__":
    ...
    with fluid.dygraph.guard():
        ...
        opt = fluid.optimizer.AdamOptimizer(learning_rate=0.0001, regularization=fluid.regularizer.L2Decay(0.00001),
                                            parameter_list=model.parameters())  # 创建优化器

        # Resnet3D
        model3d = Resnet3d(10)
        opt_3d = fluid.optimizer.AdamOptimizer(learning_rate=0.001, regularization=fluid.regularizer.L2Decay(0.00001),
                                               parameter_list=model3d.parameters())  # 创建优化器
        ...
        for epoch in range(MAX_EPOCH):
            ...
            if epoch % 100 == 0 or epoch == MAX_EPOCH - 1:
                fluid.save_dygraph(model.state_dict(), 'DpDcModel_LSTM_Conv3D_epoch{}'.format(epoch))
                fluid.save_dygraph(model3d.state_dict(), 'DpDcModel_LSTM_Conv3D_3DNet_epoch{}'.format(epoch))
