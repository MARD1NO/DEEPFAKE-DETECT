...
from CNNRNNModel4 import CNN_RNN_Model4, CNN_RNN_Model5

"""
该训练程序主要是来测试Label smooth功能是否正常运作
"""
...
if __name__ == "__main__":
    ...
    with fluid.dygraph.guard():
        model = CNN_RNN_Model5(use_label_smooth=True)
        ...
        # 训练数据读取器
        train_loader = BatchedDataLoaderv3(face_data_dir, resolution, batchsize, frame_length,
                                           use_label_smooth=use_label_smooth)
        # 验证数据读取器
        validate_loader = BatchedDataLoaderv3(val_face_data_dir, resolution, batchsize, frame_length,
                                              use_label_smooth=use_label_smooth)
        # 初始验证集准确率设置为0
        val_acc = 0
        ...
        for epoch in range(MAX_EPOCH):
            ...
            #     fluid.save_dygraph(model.state_dict(), 'DpDcModel_BiLSTM_epoch{}'.format(epoch))
