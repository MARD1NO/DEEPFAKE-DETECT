...
if __name__ == "__main__":
    ...
    with fluid.dygraph.guard():
        ...
        # opt = fluid.optimizer.Momentum(learning_rate=0.01, momentum=0.9, regularization=fluid.regularizer.L2Decay(0.000001),  parameter_list=model.parameters())  #创建优化器
        opt = fluid.optimizer.AdamOptimizer(learning_rate=0.001,  regularization=fluid.regularizer.L2Decay(0.00001),  parameter_list=model.parameters())  #创建优化器
        ...
        for epoch in range(MAX_EPOCH):
            ...
            for i, data in enumerate(train_loader()):
                ...
                # print("EPOCH{}, small epoch {}, label is {}, predict is {} loss={}".format(epoch, total_train_video_sample, frame_label, fluid.layers.argmax(output, axis=1).numpy(), loss.numpy()))
                predict = fluid.layers.argmax(output, axis=1).numpy()
                
                smallepoch += 1
                # print("accurate count is ", accurate_count)
                total_train_video_sample += batchsize
                total_acc += acc.numpy()[0]
            print("EPOCH ", epoch)
            print("total loss is ", sum(total_loss.numpy()))
            print("total acc is", total_acc/3)
            if epoch % 100 == 0 or epoch == MAX_EPOCH-1:
                fluid.save_dygraph(model.state_dict(), 'DpDcModel_small_epoch{}'.format(epoch))