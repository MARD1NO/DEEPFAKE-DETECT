...
"""
来自MogLSTM论文
使用两个卷积替代了其中的self.Q self.R矩阵
"""


class ConvMogGruCell(Layer):
    def __init__(self, in_channels, out_channels, kernel_size, mogrify_iteration=2):
        super(ConvMogGruCell, self).__init__()
...
class ConvMogGRU(Layer):
    def __init__(self, in_channels, hidden_channels, kernel_size, num_layers,
                 mogrify_iteration=2, batch_first=True, return_all_layers=False):
        super(ConvMogGRU, self).__init__()
        ...
        for i in range(0, self.num_layers):
            ...
            self.cell_list.add_sublayer(name='{}'.format(i), sublayer=ConvMogGruCell(in_channels=cur_input_dim,
...
class ConvMogBGRU(Layer):
    # Constructor
    def __init__(self, in_channels, hidden_channels,
                 kernel_size, num_layers, mogrify_iteration=2, batch_first=True):
        super(ConvMogBGRU, self).__init__()
        self.forward_net = ConvMogGRU(in_channels, hidden_channels // 2, kernel_size,
                                      num_layers, mogrify_iteration=mogrify_iteration,
                                      batch_first=batch_first)
        self.reverse_net = ConvMogGRU(in_channels, hidden_channels // 2, kernel_size,
                                      num_layers, mogrify_iteration=mogrify_iteration,
                                      batch_first=batch_first)
...
if __name__ == '__main__':
    with fluid.dygraph.guard():
        input = np.random.randn(5, 10, 32, 24, 24).astype('float32')
        x = to_variable(input)
        model = ConvMogBGRU(in_channels=32, hidden_channels=64, kernel_size=(3, 3), num_layers=2)
        out = model(x)
        print(out.shape)
